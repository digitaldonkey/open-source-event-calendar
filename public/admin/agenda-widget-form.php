...
<p class="ai1ec-limit-by-options-container" <?php if ( ! $limit_by_cat['value']) {
    ...
    <select id="<?php echo $cat_ids['id'] ?>" class="ai1ec-widget-cat-ids" name="<?php echo $cat_ids['name'] ?>[]"
            size="5" multiple="multiple" style="width: 18em; margin-left: 2em">
        <?php foreach ($cat_ids['options'] as $event_cat) : ?>
            <option value="<?php echo $event_cat->term_id; ?>"<?php if (in_array(
                $event_cat->term_id,
                $cat_ids['value']
            )) {
...
<p class="ai1ec-limit-by-container">
    ...
    <input
            ...
            type="checkbox"
            value="1" <?php if ($limit_by_tag['value']) {
            echo 'checked="checked"';
        } ?>
            onchange="this.parentElement.nextElementSibling.style.display = (this.parentElement.nextElementSibling.style.display === 'none') ? 'block' : 'none'; this.parentElement.nextElementSibling.children[0].value = '';"
    "/>
...
<p class="ai1ec-limit-by-options-container" <?php if ( ! $limit_by_tag['value']) {
    ...
    <select id="<?php echo $tag_ids['id'] ?>" class="ai1ec-widget-tag-ids" name="<?php echo $tag_ids['name'] ?>[]"
            size="5" multiple="multiple" style="width: 20em; margin-left: 2em">
        <?php foreach ($tag_ids['options'] as $event_tag) : ?>
            <option value="<?php echo $event_tag->term_id; ?>"<?php if (in_array(
                $event_tag->term_id,
                $tag_ids['value']
            )) {
                ?> selected="selected"<?php
...
<p>
    <input
            ...
            type="checkbox"
            value="1" <?php if ($show_calendar_button['value']) {
            echo 'checked="checked"';
        } ?>
            onchange="this.parentElement.nextElementSibling.style.display = (this.parentElement.nextElementSibling.style.display === 'none') ? 'block' : 'none'; this.parentElement.nextElementSibling.children[0].value = '';"
