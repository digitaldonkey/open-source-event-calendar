...
function osec_initiate_constants($osec_base_dir, $osec_base_url)
{
    ...
    /**
     * Define the maximum number of instances one event may generate.
     *
     * The timeframe only bounds rules that name no end of their own, so this is
     * what limits a rule with a large COUNT or a far UNTIL. The default leaves
     * room above the daily series the timeframe allows (about 1100 instances for
     * `+ 3years`), which is roughly five and a half years of a daily event.
     */
    if (! defined('OSEC_REOCCURRENCE_MAX_INSTANCES')) {
        define(
