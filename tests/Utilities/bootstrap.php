...
$osec_tests_dir = getenv('WP_TESTS_DIR');

if (! $osec_tests_dir) {
    $osec_tests_dir = rtrim(sys_get_temp_dir(), '/\\') . '/wordpress-tests-lib';
}

// Forward custom PHPUnit Polyfills configuration to PHPUnit bootstrap file.
$osec_phpunit_polyfills_path = getenv('WP_TESTS_PHPUNIT_POLYFILLS_PATH');
if (false !== $osec_phpunit_polyfills_path) {
    // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedConstantFound
    define('WP_TESTS_PHPUNIT_POLYFILLS_PATH', $osec_phpunit_polyfills_path);
}

if (! file_exists("{$osec_tests_dir}/includes/functions.php")) {
    // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
    echo "Could not find {$osec_tests_dir}/includes/functions.php, have you run bin/install-wp-tests.sh ?" . PHP_EOL;
    exit(1);
}

// Give access to tests_add_filter() function.
require_once "{$osec_tests_dir}/includes/functions.php";
...
tests_add_filter('muplugins_loaded', 'osec_manually_load_plugin');

// Start up the WP testing environment.
require "{$osec_tests_dir}/includes/bootstrap.php";
