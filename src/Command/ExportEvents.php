...
class ExportEvents extends CommandAbstract
{
    ...
    {
        $cat_ids = $this->_params[ 'cat_ids' ];
        $tag_ids = $this->_params[ 'tag_ids' ];
        $post_ids = $this->_params[ 'post_ids' ];
        if ( ! empty($this->_params[ 'lang' ])) {
            WpmlHelper::factory($this->app)->set_language($this->_params[ 'lang' ]);
        }
        ...
        $params = ['no_html' => $this->_params[ 'no_html' ]];
