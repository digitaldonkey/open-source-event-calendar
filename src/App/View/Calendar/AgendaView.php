...
class AgendaView extends AbstractView
{
    ...
    */
    ...
    {
        $type = $this->get_name();
        $dateSystem = UIDateFormats::factory($this->app);
        // Get localized time TODO <--- (?) Isn't it GMT?
        $timestamp = $dateSystem->current_time();
        ...
        if ( ! $view_args[ 'no_navigation' ]) {
            ...
            $pagination_links = $loader->get_file(
                'pagination.twig',
                ['links' => $pagination_links, 'data_type' => $view_args[ 'data_type' ]],
                false
            )->get_content();
    ...
    public function get_agenda_like_date_array(array $events, RequestParser $request)
    {
        ...
        foreach ($events as $event) {
            ...
            if ( ! isset($dates[ $timestamp ][ 'events' ])) {
                $dates[ $timestamp ][ 'events' ] = ['allday' => [], 'notallday' => []];
            }
...
//      )->format_to_gmt();
    ...
    {
        $view_args += $this->_request->get_dict(['page_offset', 'time_limit']);
        if (false !== $exact_date) {
