...
class AdminEventCategoryHooks extends OsecBaseClass
{
    ...
    public function edited_events_categories($term_id) : void
    {
        ...
        if (null === $term) { // term does not exist, create it
            ...
            $db->update(
                $table_name,
                ['term_color' => $tag_color_value, 'term_image' => $tag_image_value],
                ['term_id' => $term_id],
                ['%s', '%s'],
                ['%d']
            );
    ...
    public function events_categories_add_form_fields() : void
    {
        ...
        $args = [
            ...
            'section_name' => __('Category Image', OSEC_TXT_DOM),
            'label'        => __('Add Image', OSEC_TXT_DOM),
            'description'  => __('Assign an optional image to the category. Recommended size: square, minimum 400&times;400 pixels.',
	            OSEC_TXT_DOM),
            'edit'         => false,
        ];
    ...
    public static function add_actions(App $app, bool $is_admin) {
        ...
        add_action('events_categories_add_form_fields', function () use ($categoryHooks) {
            $categoryHooks->events_categories_add_form_fields();
        });

        add_action('events_categories_edit_form_fields', function ($term) use ($categoryHooks) {
            $categoryHooks->events_categories_edit_form_fields($term);
        });

        add_action('created_events_categories', function ($term_id) use ($categoryHooks) {
            $categoryHooks->created_events_categories($term_id);
        });

        add_action('edited_events_categories', function ($term_id) use ($categoryHooks) {
            $categoryHooks->edited_events_categories($term_id);
        });

        add_action('manage_edit-events_categories_columns', function ($columns) use ($categoryHooks) {
            return $categoryHooks->manage_events_categories_columns($columns);
        });

        add_action('manage_events_categories_custom_column',
