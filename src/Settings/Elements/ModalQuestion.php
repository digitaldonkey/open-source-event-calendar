...
HTML;
    ...
    {
        return isset($this->_args[ 'header_text' ]) ?
            '<h2>'.$this->_args[ 'header_text' ].'</h2>'
            : '';
    }

    private function render_id_if_present() : string
    {
        return isset($this->_args[ 'id' ]) ? "id='{$this->_args['id']}'" : '';
    }

    private function render_remove_button_if_present() : string
    {
        return isset($this->_args[ 'delete_button_text' ]) ? "<a href='#' class='ai1ec-btn remove ai1ec-btn-danger ai1ec-btn-lg'>{$this->_args['delete_button_text']}</a>" : '';
    }

    private function render_keep_button_if_present() : string
    {
        return isset($this->_args[ 'keep_button_text' ]) ? "<a href='#' class='ai1ec-btn keep ai1ec-btn-primary ai1ec-btn-lg'>{$this->_args['keep_button_text']}</a>" : '';
    }
}
